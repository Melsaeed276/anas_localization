...
<body>
  ...
  <script>
    ...
    const CATALOG_COPY = {
      en: {
        ...
        meta: {
          ...
          autoGenerateDictionaryButton: 'Auto Dictionary: {state}',
        },
    ...
    function renderMeta() {
      ...
      const autoDictionaryEnabled =
        state.meta.autoGenerateDictionary == null ? true : Boolean(state.meta.autoGenerateDictionary);
      dom.metaBadges.innerHTML = [
        ...
        '</button>',
        '<button class="pill meta-badge-button" type="button" data-action="toggle-auto-generate-dictionary">' +
          escapeHtml(
            t('meta.autoGenerateDictionaryButton', {state: autoDictionaryEnabled ? t('toggles.on') : t('toggles.off')}),
          ) +
        '</button>',
    ...
    document.addEventListener('click', async (event) => {
      ...
      if (action === 'toggle-auto-generate-dictionary') {
        if (!state.meta) return;
        const current =
          state.meta.autoGenerateDictionary == null ? true : Boolean(state.meta.autoGenerateDictionary);
        const nextValue = !current;
        try {
          await fetchJson('/api/catalog/config', {
            method: 'PATCH',
            body: JSON.stringify({autoGenerateDictionary: nextValue}),
          });
          state.meta.autoGenerateDictionary = nextValue;
          toast(t('meta.autoGenerateDictionaryButton', {state: nextValue ? t('toggles.on') : t('toggles.off')}));
          renderMeta();
        } catch (error) {
          toast(String(error || 'Failed to update config'), 'error');
        }
        return;
      }
