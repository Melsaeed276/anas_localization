...
import 'localization_metadata.dart';
...
class _MigrationVisitor extends RecursiveAstVisitor<void> {
  ...
  bool _tryRewriteTextTr(MethodInvocation node) {
    ...
    final key = firstArgument is Expression ? _extractStringLiteral(firstArgument) : null;
    if (key == null) {
  ...
  bool _tryRewriteStringTr(MethodInvocation node) {
    ...
    if (node.target is StringLiteral) {
      ...
      // ignore: unnecessary_type_check
      key = first is Expression ? _extractStringLiteral(first) : null;
      arguments = node.argumentList;
...
bool _shouldIncludeFile(String path) {
  ...
  if (lower.contains('/.dart_tool/') ||
      lower.contains(r'\.dart_tool\') ||
      lower.contains('/build/') ||
      lower.contains(r'\build\') ||
