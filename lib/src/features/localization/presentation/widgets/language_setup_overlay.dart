...
/// Package-internal key used to reference the active [AnasLanguageSetupOverlay]
/// state without a static instance field.
///
...
class AnasLanguageSetup {
  ...
  static bool tryChangeLanguage(Locale targetLocale) {
    final state = anasLanguageSetupOverlayKey.currentState;
    if (state is _AnasLanguageSetupOverlayState) {
      state._showLanguageSetupOverlay(targetLocale: targetLocale);
      return true;
    }
    return false;
  }
  ...
  static bool get isSetupOverlayAvailable => anasLanguageSetupOverlayKey.currentState != null;
}
...
class AnasLocalizationWithSetup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnasLocalization(
      ...
      // Disable the built-in overlay wrapping — we provide our own below when
      // enableSetupScreen is true, avoiding a double AnasLanguageSetupOverlay.
