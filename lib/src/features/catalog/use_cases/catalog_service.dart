...
class CatalogService {
  ...
  static bool _dictionaryGenerationScheduled = false;

  void _triggerDictionaryGeneration() {
    if (!config.autoGenerateDictionary) {
      return;
    }

    if (_dictionaryGenerationScheduled) {
      return;
    }
    _dictionaryGenerationScheduled = true;

    unawaited(() async {
      // Coalesce rapid edits into a single generation run.
      await Future<void>.delayed(const Duration(milliseconds: 250));
      try {
        await Process.start(
          'dart',
          const ['run', 'anas_localization:localization_gen'],
          workingDirectory: projectRootPath,
          runInShell: true,
          mode: ProcessStartMode.detached,
        );
      } catch (_) {
        // Intentionally ignore failures; catalog writes should still succeed.
      } finally {
        _dictionaryGenerationScheduled = false;
      }
    }());
  }
  ...
  Future<CatalogMeta> loadMeta() async {
    ...
    return CatalogMeta(
      ...
      autoGenerateDictionary: config.autoGenerateDictionary,
    );
  }
